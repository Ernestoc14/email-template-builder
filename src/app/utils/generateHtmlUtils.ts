...
const makeHTML = (template: masterJSONType, lang: string,) => {
    const langProp: "esProps" | "enProps" | "ptProps" | "frProps" = `${lang.toLowerCase() as "es" | "en" | "pt" | "fr"
        }Props`;
    ...
    const isFullWidth = template.boxAzul.components[0]?.componentId.includes("Images")
